...
function quantile( p, options ) {
	...
	if ( isNumber( p ) ) {
		return quantile1( p, opts.mu, opts.s );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, opts.mu, opts.s );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.mu, opts.s );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.mu, opts.s, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.mu, opts.s, opts.accessor );
		}
		return quantile2( out, p, opts.mu, opts.s );
	}
	return NaN;
